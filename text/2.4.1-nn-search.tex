...
$(\mathcal{X}_i, \mathcal{Y}_i)_{i \le i \le n}$ a training set with their
...
$(matcal{X}_{(i)}, Y_{(i)}) \, 1\le i\le n$ so that the
$\matcal{X}_{(i)}$'s are re-arranged in increasing distance from $x$,
$d(x, matcal{X}_{(1)}) \d(x, dathcal{X}_{(2)}) \le ... d(x, \mathcal{X}_{(n)})$,
...
use the distance $\mathb{L}^\infty$, we may visualize $k(x)$ as the set of all
functions within a band  of radius $r$ around $x$.

In the figure \ref{NNSEARCH} there are shown the neighborhoods with these two different
approaches.

\begin{figure}[neighborhoods using the distance \mathbb{L}^\infty]{FIG:NNSEARCH}{Amplitude and phase variability}
  \subfigure[SBFIG:NNSEARCH1]{K-nearest neighbors}{\image{7cm}{}{0-empty}} \quad
  \subfigure[SBFIG:NNSEARCH2]{Radius-nearest neighbors}{\image{7cm}{}{0-empty}}
\end{figure}
